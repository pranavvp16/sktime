...
class MOIRAIForecaster(_BaseGlobalForecaster):
    ...
    def _get_moirai_cache_key(
        self, prediction_length, feat_dynamic_real_dim, past_feat_dynamic_real_dim
    ):
    ...
    def _load_model_via_multiton(
        ...
        cache_key = self._get_moirai_cache_key(
            prediction_length, feat_dynamic_real_dim, past_feat_dynamic_real_dim
        )
    ...
    def _predict_quantiles(self, fh, X=None, alpha=None, y=None):
        ...
        for forecast in forecast_list:
            time_index = (
                forecast.mean_ts.index if hasattr(forecast, "mean_ts") else None
            )
        ...
        if len(quantile_dfs) > 0:
            ...
            if forecast_list[0].item_id is not None:
                result = result.set_index(
                    [df_config["item_id"], df_config["timepoints"]]
                )
                ...
                var_name = (
                    self._target_name[0]
                    if self._target_name is not None and len(self._target_name) > 0
                    else 0
                )
                if var_name is None:
                    var_name = 0
                ...
                result = result.set_index("timepoints")
                result = result.pivot_table(
                    values="quantile", index="timepoints", columns="alpha"
                )
                var_name = (
                    self._target_name[0]
                    if self._target_name is not None and len(self._target_name) > 0
